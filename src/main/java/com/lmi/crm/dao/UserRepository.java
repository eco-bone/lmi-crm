...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    List<User> findAssociatesByLicensee(@Param("licenseeId") Integer licenseeId, @Param("role") UserRole role, @Param("status") UserStatus status);

    @Query("SELECT u FROM User u WHERE (:role IS NULL OR u.role = :role) AND (:status IS NULL OR u.status = :status)")
    List<User> findByOptionalFilters(@Param("role") UserRole role, @Param("status") UserStatus status);
}
